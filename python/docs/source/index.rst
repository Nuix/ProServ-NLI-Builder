ProServe-NLI-Builder documentation
==================================
...
The above code would produce an NLI that will ingest into a Nuix case to produce the following evidence structure:
...
CSVEntry
    ...
    that file in the NLI file.  Add a CSVEntry to an NLI using the ``nuix_nli_lib.edrm.NLIGenerator#add_entry(...)``
    method.  This class implements a special ``#add_to_builder(...)`` method which will be called automatically and adds
    a CSVRowEntry instance for each row of data to the CSV to the NLI.  This class also extends the
...
JSONFileEntry
    ...
    structure.  As with CSVEntry, this class should be added to the NLI with ``nuix_nli_lib.edrm.NLIGenerator#add_antry(...)``
    so the ``#add_to_builder`` method will be used to also add child items for its content.  This class also extends the
    FileEntry to allow it to both serve as a Native, and act as a Container for other items (it will become parent to
    exactly one item, a JSONValueEntry, JSONArrayEntry, or JSONObjectEntry depending on the file's content).
